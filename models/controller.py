    ...
    def execute_program(self, program_name: str) -> bool:
        ...
        for i, step in enumerate(steps):
            ...
            try:
                ...
                # Step 7: Start wash motor (D01) - DIDO PROTOCOL (no camera detection)
                if step_num == 7:
                    print("\n🚀 [STEP 7] Starting WASH station motor (D01) - DIDO PROTOCOL...")
                    success, response = self.robot.wash_motor_on()
                    if not success:
                        self.log_error(f"Wash motor activation failed at step 7: {response}")
                        self.robot.emergency_stop_motors()
                        self.robot.buzzer()
                        return False
                    print("✓ Wash motor ON - motor should run now")
                
                # Step 9: Stop wash motor (D01)
                if step_num == 9:
                    print("\n🛑 [STEP 9] Stopping WASH station motor (D01)...")
                    success, response = self.robot.wash_motor_off()
                    if not success:
                        self.log_error(f"Wash motor deactivation failed at step 9: {response}")
                        self.robot.emergency_stop_motors()
                        return False
                    print("✓ Wash motor OFF - motor should stop now")
                
                # Step 11: Start rinse motor (D03) - DIDO PROTOCOL (no camera detection)
                if step_num == 11:
                    print("\n🚀 [STEP 11] Starting RINSE station motor (D03) - DIDO PROTOCOL...")
                    success, response = self.robot.rinse_motor_on()
                    if not success:
                        self.log_error(f"Rinse motor activation failed at step 11: {response}")
                        self.robot.emergency_stop_motors()
                        self.robot.buzzer()
                        return False
                    print("✓ Rinse motor ON - motor should run now")
                
                # Step 13: Stop rinse motor (D03)
                if step_num == 13:
                    print("\n🛑 [STEP 13] Stopping RINSE station motor (D03)...")
                    success, response = self.robot.rinse_motor_off()
                    if not success:
                        self.log_error(f"Rinse motor deactivation failed at step 13: {response}")
                        self.robot.emergency_stop_motors()
                        return False
                    print("✓ Rinse motor OFF - motor should stop now")
                ...
                if step_num in detection_checkpoints:
                    ...
                    if not cup_detected:
                        ...
                        # 2. TRIGGER ARM ALARM (G01 signal)
                ...
                # Safety: Stop motors and alarm on any error
                print(f"\n🚑 ERROR AT STEP {step_num}: Emergency protocol activated...")
                print(f"   Exception: {e}")
